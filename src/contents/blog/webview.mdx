...
앞서 봤던 자바스크립트 inject나 웹뷰와 앱 사이의 통신 방법을 통해서도 이를 해결할 순 있겠지만 Cookie, Web Storage 동기화를 해주는 방식이 더 좋을 것 같다.

웹에서는 cookie나 web storage에 저장된 인증 정보를 헤더에 담아서 api 요청을 보내기 때문에 지속적으로 웹뷰에서 인증된 유저의 정보를 백엔드로부터 받아야하는 경우에 cookie나 web storage에 인증 정보를 담아주어야 할 것이다. 만약 앱에 있는 데이터를 웹뷰에서 단순히 렌더링해주는 상황 같은 경우에는 inject하거나 브릿지로 전달하는 것도 나쁘진 않을 것 같다.
